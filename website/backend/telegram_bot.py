...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if donor:
        await update.message.reply_html(
            ...
            f"🌐 <a href='{WEBSITE_URL}/pages/donor-dashboard.html'>Перейти на сайт</a>"
        )
    else:
        keyboard = [
        ...
        await update.message.reply_html(
            ...
            f"<b>Для подтверждения регистрации введите 6-значный код,</b>\n"
            f"который вы получили на сайте.\n\n"
            f"<b>Пример:</b> <code>123456</code>\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"Через этого бота вы будете получать уведомления о:\n"
            f"• Срочных запросах крови вашей группы\n"
            f"• Статусе ваших откликов\n"
            f"• Напоминаниях о донациях\n\n"
            f"Ещё нет аккаунта? Зарегистрируйтесь на сайте! 👇",
    ...
    if text.isdigit():
        ...
        if len(text) == 6:
            ...
            await update.message.reply_html(
                ...
                f"Получить код можно на сайте в личном кабинете."
            )
    else:
        ...
        await update.message.reply_html(
            ...
            "Пока я могу только:\n"
            "• Подтвердить вашу регистрацию (введите 6-значный код с сайта)\n"
...
async def verify_code(update: Update, context: ContextTypes.DEFAULT_TYPE, code: str):
    ...
    if existing:
        await update.message.reply_html(
            ...
            f"Если это не вы — обратитесь в поддержку на сайте:\n"
            f"🌐 {WEBSITE_URL}"
        )
        return
    ...
    if not link_data:
        await update.message.reply_html(
            ...
            "Проверьте код на сайте и попробуйте ещё раз.\n"
